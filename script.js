...
<button id="bigDelbtn">Delete Slideshow(↓)</button>
</a>
<div class="item" id = ${name}>
<a href='/load?pageName=${link.replace('.html', '').replace('/templates/', '')}'>
  <img src = ${image} /></a>
...
function run2() {
  ...
  //https://javarevisited.blogspot.com/2017/01/string-replaceall-example-how-to-replace-all-characters-and-substring.html#axzz7RDGuEydv
...
function load2(dict) {
  ...
  for (var i = 0; i < json_dict.length; i+=2) {
     //https://stackoverflow.com/questions/3214886/javascript-replace-only-replaces-first-match
    //https://javarevisited.blogspot.com/2017/01/string-replaceall-example-how-to-replace-all-characters-and-substring.html#axzz7RDGuEydv
    newBox(json_dict[i], '/templates/' + json_dict[i+1].replaceAll(' ', '_') + '-slideshow.html', json_dict[i+1])
  }  
}
...
function load2Admin(dict) {
  ...
  for (var i = 0; i < json_dict.length; i+=2) {
     //https://stackoverflow.com/questions/3214886/javascript-replace-only-replaces-first-match
    //https://javarevisited.blogspot.com/2017/01/string-replaceall-example-how-to-replace-all-characters-and-substring.html#axzz7RDGuEydv
    newBoxAdmin(json_dict[i], '/templates/' + json_dict[i+1].replaceAll(' ', '_') + '-slideshow.html', json_dict[i+1])
  }  
}
