...
function loadSlides(dict) {
  ...
  for (var i = 0; i < json_dict.length; i+=5) {
    addImage(json_dict[i], json_dict[i+4])
  }  
}

/* When the user clicks on the button,
toggle between hiding and showing the dropdown content */
function myFunction() { document.getElementById("myDropdown2").classList.toggle("show");
}
...
function run2() {
  ...
  var n = document.getElementsByClassName("fade").length-1;
  addImage(image, n)
...
function loadDropDown(dict) {
  if (dict != '') {
    json_dict = dict.split("|")
    for (var i = 0; i < json_dict.length; i+=5) {
      addDropDownImage(json_dict[i], json_dict[i+1], json_dict[i+4], json_dict[i+2], json_dict[i+3])
    }
  }
}
